...
class Player(commands.Cog):
    ...
    async def play(self, ctx, *, song=None):
        ...
        if ctx.voice_client is None:
            return await ctx.send("Eu preciso estar em um canal de voz para tocar.")
        
        #quando a musica nao for url
        if not ("youtube.com/watch?" in song or "https://youtu.be/" in song):
            await ctx.send("Procurando a música, isso pode levar alguns segundos.")

            result = await self.search_song(1, song, get_url=True)

            if result is None:
                return await ctx.send("Desculpe, não consegui achar.")
        ...
        if ctx.voice_client.source is not None:
            ...
            if queue_len < 10:
                ...
                return await ctx.send(f"No momento estou tocando uma música, sua música foi adicionada na fila na posição: {queue_len+1}.")
            else:
                return await ctx.send("Desculpe, eu só consigo colocar até 10 músicas na fila.")
    ...
    async def search(self, ctx, *, song=None):
        if song is None: return await ctx.send("Você esqueceu de colocar a música.")

        await ctx.send("Procurando, isso pode levar alguns segundos.")
